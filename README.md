...
- uses: elioseverojunior/rust-toolchain@v0.1
  with:
    ...
    cache-key-hash:
    # A hash of the dependency set, required when `cache` is true. Pass the workflow expression `hashFiles('**/Cargo.lock')` wrapped in the usual interpolation braces — `hashFiles` is a workflow expression function that a Node action cannot call, and using GitHub's own value keeps these keys interoperable with caches you already have. Without it the keys never change: they hit exactly on every run and serve the same crates for the life of the repository. The README's caching section shows the full step. Note this text deliberately avoids writing the interpolation braces literally: an expression inside action metadata is not an expression context, and `act` rejects the whole action with "expressions are not allowed here" before any code runs.
    #
    # Required: false
    # Default: ""

    cache-key-suffix:
    # An optional discriminator added to every cache key, e.g. a job name. Omitting it collapses the slot rather than leaving an empty segment, so the key reads `registry-Linux-X64-<hash>` and not `registry-Linux-X64--<hash>`. You do not need it to separate jobs that differ in `RUSTFLAGS`, or in any other variable whose name begins with `CARGO_`, `CC`, `CFLAGS`, `CXX`, `CMAKE` or `RUST` — every one of those is hashed into the `build` key already, so such jobs no longer share one. (Prefixes rather than wildcards on purpose: the README table is generated from this text as HTML, where a pair of asterisks reads as emphasis and swallows the underscores between them.) Reach for it when the thing that distinguishes two jobs is not an environment variable at all: two jobs building the same crate with the same flags for different purposes, say, or one you want kept out of another's restore ladder. May not contain a comma or whitespace.
    #
    # Required: false
    # Default: ""
