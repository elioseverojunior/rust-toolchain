...
| `json`                 | <p>Every output above as one JSON object, natively typed — <code>targets</code> and <code>components</code> are real arrays and <code>set-rustup-toolchain</code> is a real boolean, so <code>fromJSON(steps.&lt;id&gt;.outputs.json).targets</code> drops straight into a matrix. It also adds two provenance blocks the flat outputs cannot carry: <code>inputs</code>, the action inputs verbatim, and <code>toml</code>, the parsed <code>rust-toolchain.toml</code> before merging. Together they say where every resolved value came from — absent toml scalars are <code>null</code>, unset inputs are empty strings.</p> |
...
It also adds two blocks the flat outputs cannot express — `inputs`, the action
inputs verbatim, and `toml`, the parsed `rust-toolchain.toml` before merging.
Together they say where each resolved value came from: unset inputs are empty
strings, absent toml scalars are `null`.
