...
Install cargo binaries alongside the toolchain, cached in the `bin` layer:

```yaml
- uses: elioseverojunior/rust-toolchain@v0.5
  with:
    cargo-tools: cargo-nextest, cargo-deny@0.16.1, ignorefile-cli@0.1.0:ign
```

Each entry is `<name>[@<version>][:<binary>]`. A bare name means `latest` and is
resolved through crates.io; a pinned version never contacts the registry at all,
which is what keeps an outage from touching it.

The `:<binary>` suffix names the executable to version-probe. The action decides
whether a tool is already installed by running `<name> --version`, so a crate
whose executables are not named after it is never recognised — `ignorefile-cli`
installs `ign` and `ignorefile`, so the probe finds nothing, treats the tool as
absent, and rebuilds it from source on every run even when the `bin` cache had
just restored it. Naming the binary is what closes that loop.

It redirects the probe and nothing else. The crate name is still what gets
installed, and neither the `cargo-tools` output nor the `bin` cache key changes,
so adding the suffix to a workflow you already run still hits the cache entry
that workflow already has.
...
| `msrv-fallback`        | <p>Whether to derive the toolchain from <code>Cargo.toml</code>'s <code>rust-version</code> when neither the <code>toolchain</code> input nor <code>rust-toolchain.toml</code> names a channel (default false). Off by default because <code>rust-version</code> is a floor, not a pin: turning it on silently moves a repository that has one from <code>stable</code> to its MSRV.</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `false`  | `false`              |
| `msrv-check`           | <p>What to do when the installed toolchain is older than the highest <code>rust-version</code> in the resolved dependency graph: <code>off</code>, <code>warn</code> (default) or <code>error</code>. The graph is what binds, not the crate's own declaration — a crate can declare 1.79 while pinning a dependency that needs 1.95. Covers every directory named by <code>cache-workspaces</code> (default the checkout root alone), not the root manifest only, so a monorepo whose crates live under their own manifests is still checked. A directory with no <code>Cargo.toml</code> is skipped silently. A check that cannot run, for want of a lockfile or a manifest anywhere, always warns and never fails, whatever this is set to.</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `false`  | `warn`               |
| `msrv-install`         | <p>Whether to also install the crate's declared MSRV (<code>rust-version</code> from the workspace-root <code>Cargo.toml</code>, the same value published as the <code>msrv</code> output) alongside the resolved toolchain (default false), so a later step can run <code>cargo +&lt;msrv&gt; check</code> without its own <code>rustup toolchain install</code>. Installs the declared floor, never <code>msrv-effective</code> — the point is proving the crate builds at the version it advertises, so a dependency that has pushed the real floor higher should fail that check rather than being papered over. Installed at the same resolved <code>profile</code> as the primary toolchain — never a hardcoded one — with that profile's implied components (e.g. <code>clippy</code>, <code>rustfmt</code> under <code>default</code>) added by name afterwards, so <code>cargo +&lt;msrv&gt; clippy</code> works the same way it does on the primary toolchain. Never becomes the active toolchain: no <code>rustup default</code> and no <code>RUSTUP_TOOLCHAIN</code> export, so it is reachable only via <code>cargo +&lt;msrv&gt;</code>. Skipped, with a note in the log, when the declared MSRV already equals the resolved channel; does nothing at all, silently, when no <code>rust-version</code> is declared anywhere. Off by default because the installed toolchain is inert until a workflow step invokes it directly, so defaulting it on would cost every user a download for something most never call. Unlike <code>msrv-check</code>, a failed install here fails the job: this toolchain was explicitly requested, so an unresolvable channel is a real configuration error, not something to warn past.</p>                              | `false`  | `false`              |
| `cargo-tools`          | <p>Cargo-installed tools to make available, comma, space or newline separated. Each entry is <code>&lt;name&gt;</code>, <code>&lt;name&gt;@&lt;version&gt;</code> or <code>&lt;name&gt;@&lt;version&gt;:&lt;binary&gt;</code>, where the version may be <code>latest</code>; a bare name means <code>latest</code>. A tool already present at the requested version is left alone, so a restored <code>bin</code> cache layer makes most runs install nothing. The optional <code>:&lt;binary&gt;</code> suffix names the executable to check for when the crate does not install one named after itself — <code>ignorefile-cli</code> ships <code>ign</code> and <code>ignorefile</code>, so without <code>ignorefile-cli@0.1.0:ign</code> the check finds nothing, treats the tool as absent and rebuilds it from source on every run even on a <code>bin</code> cache hit. It redirects that check and nothing else: the crate name is still what gets installed, and both the <code>cargo-tools</code> output and the <code>bin</code> cache key are unchanged by it, so adding the suffix to an existing workflow still hits the entry that workflow already has. A pinned version never contacts crates.io, which is what keeps a registry outage from affecting it. An unpinned one is resolved there, and if that fails the action falls back to a binary of the same name when one is present, warns, and reports its version as <code>unknown</code>. With nothing to fall back to it fails, because a missing tool breaks the job either way and failing here says why. The binaries are cached by the <code>bin</code> layer, which is enabled by default; rustup's own shims are excluded from it, so a toolchain bump does not reinstall your tools.</p> | `false`  | `""`                 |
...
| `msrv`                 | <p>The <code>rust-version</code> declared by the workspace-root <code>Cargo.toml</code>, or an empty string. The root manifest only, never a maximum over workspace members.</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `msrv-effective`       | <p>The highest <code>rust-version</code> across every package in the resolved dependency graph, or an empty string. Resolved over every directory named by <code>cache-workspaces</code>, not the root manifest only — unlike <code>msrv</code> below, which stays root-only. Use this, not <code>msrv</code>, to choose a toolchain: the two differ whenever a dependency outruns the crate's own declared floor. Empty under <code>msrv-check: off</code>, since the check that computes it never ran — also empty when the check ran but could not complete (no <code>Cargo.toml</code> in any directory, no lockfile, or no package in the graph declares a <code>rust-version</code>), the same as <code>off</code>. Build a matrix from this output only when <code>msrv-check</code> is <code>warn</code> or <code>error</code>.</p>                                                         |
| `msrv-source`          | <p>Where <code>msrv</code> was read from — <code>cargo-toml</code>, <code>workspace-inherit</code>, or <code>none</code>. <code>workspace-inherit</code> covers two shapes: a member that opted in with <code>rust-version.workspace = true</code>, and a virtual manifest (no <code>[package]</code>), whose <code>[workspace.package]</code> table is the declaration itself rather than something inherited.</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- uses: elioseverojunior/rust-toolchain@v0.5
  with:
    ...
    msrv-fallback:
    # Whether to derive the toolchain from `Cargo.toml`'s `rust-version` when neither the `toolchain` input nor `rust-toolchain.toml` names a channel (default false). Off by default because `rust-version` is a floor, not a pin: turning it on silently moves a repository that has one from `stable` to its MSRV.
    #
    # Required: false
    # Default: false

    msrv-check:
    # What to do when the installed toolchain is older than the highest `rust-version` in the resolved dependency graph: `off`, `warn` (default) or `error`. The graph is what binds, not the crate's own declaration — a crate can declare 1.79 while pinning a dependency that needs 1.95. Covers every directory named by `cache-workspaces` (default the checkout root alone), not the root manifest only, so a monorepo whose crates live under their own manifests is still checked. A directory with no `Cargo.toml` is skipped silently. A check that cannot run, for want of a lockfile or a manifest anywhere, always warns and never fails, whatever this is set to.
    #
    # Required: false
    # Default: warn

    msrv-install:
    # Whether to also install the crate's declared MSRV (`rust-version` from the workspace-root `Cargo.toml`, the same value published as the `msrv` output) alongside the resolved toolchain (default false), so a later step can run `cargo +<msrv> check` without its own `rustup toolchain install`. Installs the declared floor, never `msrv-effective` — the point is proving the crate builds at the version it advertises, so a dependency that has pushed the real floor higher should fail that check rather than being papered over. Installed at the same resolved `profile` as the primary toolchain — never a hardcoded one — with that profile's implied components (e.g. `clippy`, `rustfmt` under `default`) added by name afterwards, so `cargo +<msrv> clippy` works the same way it does on the primary toolchain. Never becomes the active toolchain: no `rustup default` and no `RUSTUP_TOOLCHAIN` export, so it is reachable only via `cargo +<msrv>`. Skipped, with a note in the log, when the declared MSRV already equals the resolved channel; does nothing at all, silently, when no `rust-version` is declared anywhere. Off by default because the installed toolchain is inert until a workflow step invokes it directly, so defaulting it on would cost every user a download for something most never call. Unlike `msrv-check`, a failed install here fails the job: this toolchain was explicitly requested, so an unresolvable channel is a real configuration error, not something to warn past.
    #
    # Required: false
    # Default: false

    cargo-tools:
    # Cargo-installed tools to make available, comma, space or newline separated. Each entry is `<name>`, `<name>@<version>` or `<name>@<version>:<binary>`, where the version may be `latest`; a bare name means `latest`. A tool already present at the requested version is left alone, so a restored `bin` cache layer makes most runs install nothing. The optional `:<binary>` suffix names the executable to check for when the crate does not install one named after itself — `ignorefile-cli` ships `ign` and `ignorefile`, so without `ignorefile-cli@0.1.0:ign` the check finds nothing, treats the tool as absent and rebuilds it from source on every run even on a `bin` cache hit. It redirects that check and nothing else: the crate name is still what gets installed, and both the `cargo-tools` output and the `bin` cache key are unchanged by it, so adding the suffix to an existing workflow still hits the entry that workflow already has. A pinned version never contacts crates.io, which is what keeps a registry outage from affecting it. An unpinned one is resolved there, and if that fails the action falls back to a binary of the same name when one is present, warns, and reports its version as `unknown`. With nothing to fall back to it fails, because a missing tool breaks the job either way and failing here says why. The binaries are cached by the `bin` layer, which is enabled by default; rustup's own shims are excluded from it, so a toolchain bump does not reinstall your tools.
    #
    # Required: false
    # Default: ""
