...
| `msrv-check`           | <p>What to do when the installed toolchain is older than the highest <code>rust-version</code> in the resolved dependency graph: <code>off</code>, <code>warn</code> (default) or <code>error</code>. The graph is what binds, not the crate's own declaration — a crate can declare 1.79 while pinning a dependency that needs 1.95. A check that cannot run, for want of a lockfile or a manifest, always warns and never fails, whatever this is set to.</p>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `false`  | `warn`               |
...
| `msrv-effective`       | <p>The highest <code>rust-version</code> across every package in the resolved dependency graph, or an empty string. Use this, not <code>msrv</code>, to choose a toolchain: the two differ whenever a dependency outruns the crate's own declared floor. Empty under <code>msrv-check: off</code>, since the check that computes it never ran — also empty when the check ran but could not complete (no <code>Cargo.toml</code>, no lockfile, or no package in the graph declares a <code>rust-version</code>), the same as <code>off</code>. Build a matrix from this output only when <code>msrv-check</code> is <code>warn</code> or <code>error</code>.</p>                                                                                                                                                                                                                                    |
...
- uses: elioseverojunior/rust-toolchain@v0.5
  with:
    ...
    msrv-check:
    # What to do when the installed toolchain is older than the highest `rust-version` in the resolved dependency graph: `off`, `warn` (default) or `error`. The graph is what binds, not the crate's own declaration — a crate can declare 1.79 while pinning a dependency that needs 1.95. A check that cannot run, for want of a lockfile or a manifest, always warns and never fails, whatever this is set to.
    #
    # Required: false
    # Default: warn
