...
export default [
  {
    ...
    ignores: [
      "dist/**",
      // Docusaurus' generated output. `build/` is the production bundle and
      // `.docusaurus/` the dev-time codegen; both are minified or machine-
      // written, and linting them reports hundreds of no-undef/no-redeclare
      // errors against webpack runtime chunks nobody edits. Flat config does
      // not read .gitignore, so listing them there is not enough.
      "build/**",
      ".docusaurus/**",
      "coverage/**",
      "node_modules/**",
