...
| 10  | `cache-save-if` defaults to `true` — every branch saves                     | User's call, matching `rust-cache`. Consequence: budget enforcement is the only remaining defence against eviction, so it carries more weight.                             |
...
| Every branch saves (decision 10), so PR-scoped entries consume the shared budget and are discarded when the PR closes      | `cache-budget` and skip-save-on-exact-hit are the available levers. Document `cache-save-if` with a default-branch-only recipe for repositories that hit the ceiling.                                                                                                                             |
