...
touches `@actions/cache` **or the network**: everything below it depends on the
`CacheClient` interface in `src/cache/client.ts` and the `RegistryClient`
interface in `src/tools.ts`, both type-only, so the tests never load the real
SDK and never make an HTTP request. The two ports exist for the same reason and
are wired the same way — the real `@actions/cache` adapter and the real
crates.io `fetch` adapter are both built in `src/index.ts`, which nothing
imports and the coverage gate cannot see. And `src/action.ts` fans out to the cache modules rather than the
reverse — no cache module imports the orchestrator, which is what lets each one
be tested against plain values.

The barrel `src/lib.ts` re-exports all twenty-two library modules; only its edge
to `src/action.ts` is drawn, since the other twenty-one would say nothing but
"the barrel exports everything". It deliberately does **not** re-export
...
flowchart TD
    ...
    Y --> A[read rust-toolchain.toml,<br/>read Cargo.toml when msrv-fallback is on]
    A --> B[merge inputs, validate,<br/>Cargo.toml rust-version as the last fallback]
    B --> C[resolveChannel]
...
| `src/errors.ts`          | Module                              | `describeError`                                                                                                                                                                                                                                                                                                            | Renders a caught `unknown` as a message. Extracted because the `error instanceof Error ? error.message : String(error)` check was written out nine times across `action.ts`, `cache/lifecycle.ts` and `core.ts`                                                                                                                                                                                                                                                                                                                               |
...
| `src/cache/layers.ts`    | Module                              | `CACHE_LAYER_IDS`, `CacheLayerId`, `parseCacheLayers`                                                                                                                                                                                                                                                                      | The canonical layer list (`registry`, `build`, `bin`) and the `cache-layers` input parser                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `src/cache/paths.ts`     | Module (node:path)                  | `parseWorkspaces`, `registryPaths`, `buildPaths`, `binPaths`, `RUSTUP_SHIMS`, `Workspace`                                                                                                                                                                                                                                  | Reads `cache-workspaces` into resolved `<manifest-dir> -> <target-dir>` mappings, refusing anything outside the checkout; the `registry` layer's fixed paths; the `build` layer's paths as a files-only glob set — `<target>/**` plus `!<target>/**/incremental/**`, `!<target>/**/examples/**` and the two directory negations that keep the manifest free of directories; the `bin` layer's paths the same way, negating each of the fourteen `RUSTUP_SHIMS` names and its `.exe` twin so a toolchain bump cannot invalidate the tool cache |
...
| `src/tools.ts`           | Module (node:crypto)                | `parseToolSpecs`, `resolveToolVersions`, `hashToolSet`, `ensureTools`, `parseToolVersion`, `RegistryClient`, `ResolvedTool`, `ToolResolution`, `UNRESOLVED_VERSION`                                                                                                                                                        | Owns `cargo-tools` end to end: parses and validates `<name>[@<version>][:<binary>]` entries before any command runs, resolves `latest` through the `RegistryClient` port with backoff, digests the resolved set into the `bin` key, and installs only what the restore did not supply. A pinned version never reaches the client, so a registry outage cannot affect it; the `:<binary>` suffix redirects the version probe only, and is excluded from both the key and the output                                                            |
| `src/msrv.ts`            | Module (smol-toml)                  | `parseVersion`, `compareVersions`, `parseMsrvPolicy`, `parseCargoManifest`, `effectiveMsrv`, `evaluateMsrv`, `describeVerdict`, `Version`, `MsrvPolicy`, `MsrvSource`, `ManifestMsrv`, `MsrvRequirement`, `MsrvVerdict`                                                                                                    | Pure functions, no I/O: parses a `Cargo.toml`'s `rust-version` (including workspace inheritance and the `rust-version.workspace = true` trap), compares versions numerically, and finds the maximum requirement across `cache/metadata.ts`'s resolved graph. `evaluateMsrv` never fails a build itself — it returns a verdict `action.ts` decides what to do with                                                                                                                                                                             |
| `src/cache/metadata.ts`  | Module                              | `parsePackageSet`, `parsePackageMsrv`, `MetadataReader`, `PackageSet`, `PackageId`, `PackageMsrv`                                                                                                                                                                                                                          | Reads `cargo metadata --format-version 1 --locked` into the packages a workspace still resolves to, with its own crates separated (`parsePackageSet`), and separately into every package's declared `rust_version` (`parsePackageMsrv`, lenient where the former is strict — a malformed entry costs its own contribution, not the whole check). Matches `workspace_members` against package ids verbatim rather than parsing them, since cargo spells ids at least three ways                                                                |
| `src/cache/prune.ts`     | Module                              | `PrunePolicy`, `parsePrunePolicy`, `readFingerprints`, `computeKeepSet`, `Fingerprints`, `DirReader`                                                                                                                                                                                                                       | Decides which files under `target/` the archive carries. Recovers ownership from the `<name>-<hash>` fingerprint directories cargo already writes — authoritative, where `rust-cache` strips a trailing `-$hash` off a filename and guesses. Nothing here deletes; the keep-set selects what is linked into the stage                                                                                                                                                                                                                         |
| `src/cache/stage.ts`     | Module                              | `STAGE_DIR_NAME`, `buildStageRoots`, `registryStageRoot`, `stagePaths`, `stagedLocation`, `unstagedLocation`, `stageFiles`, `unstageFiles`, `StageRoot`, `StageFs`, `StageOutcome`                                                                                                                                         | Keeps a pruned layer's paths array constant. `@actions/cache` matches an entry on `(key, sha256(paths.join("\|") \| compressionMethod \| salt))`, so narrowing the array to a keep-set writes entries no restore can ever find — the restore does not yet know the content. A staged layer archives one fixed directory per tree instead: `stageFiles` hard-links the keep-set in before a save, `unstageFiles` moves it back after a restore, and the working tree is never modified                                                         |
| `src/cache/fs.ts`        | Module (node:fs)                    | `walkFiles`, `nodeStatFs`, `nodeStageFs`                                                                                                                                                                                                                                                                                   | The real `node:fs` implementations of `StatFs` and `StageFs`. They lived in `src/index.ts` until a fake that disagreed with `readdirSync` about a missing directory cost every pruned layer its save; nothing here vendors an SDK or touches the network, so the coverage exemption they inherited by proximity was never earned. `fs.test.ts` holds them to a real temp directory — the hard link's shared inode and mtime included                                                                                                          |
...
| `src/lib.ts`             | Barrel                              | re-exports `action`, `builder`, `cache/budget`, `cache/client`, `cache/env`, `cache/fs`, `cache/inputs`, `cache/keys`, `cache/layers`, `cache/lifecycle`, `cache/metadata`, `cache/paths`, `cache/prune`, `cache/stage`, `cache/summary`, `config`, `core`, `errors`, `inputs`, `msrv`, `outputs`, `tools` — never `index` | The library surface under one specifier, for consumers that would rather import `@rust-toolchain` than twenty-two modules                                                                                                                                                                                                                                                                                                                                                                                                                     |

`src/lib.ts` is the barrel; individual modules can still be imported directly
and are cheaper, since the barrel loads all twenty-two.
...
| `src/msrv.test.ts`            | `parseVersion`/`compareVersions` — the `1.9` vs `1.10` numeric case; `parseMsrvPolicy`; `parseCargoManifest` — `[package]`, `[workspace.package]`, the `rust-version.workspace = true` inheritance trap, and rejection of invalid TOML; `effectiveMsrv`/`evaluateMsrv`/`describeVerdict` — the `cargo-binstall`/`vergen` fixture where a dependency outruns the crate's own declaration | 100%                              |
...
and saves against them. Three layers exist today, defined in
...
- **`bin`** — the cargo-installed tools named by `cargo-tools`, under
  `$CARGO_HOME/bin`. Its key is `bin-<os>-<arch>-<toolSetHash>`, and what it
  leaves out is the interesting part. No `<lockHash>`: a tool is installed from
  crates.io and has nothing to do with the workspace's dependency set, which is
  also why a `bin`-only `cache-layers` needs no `cache-key-hash` at all. No
  `<specCacheKey>`: the point of the layer is that a compiler bump does not
  reinstall tools that did not change. And no `<suffix>`: two jobs resolving
  the same tool set need byte-identical binaries, so splitting them apart would
  cost sharing and buy nothing. Unlike `build`, its ladder does fall back to
  the bare `bin-<os>-<arch>-` prefix, because a stale tool binary is still a
  real binary and `ensureTools` version-probes it before trusting it.

  A `cargo-tools` entry's optional `:<binary>` suffix is left out too, and that
  omission is load-bearing rather than tidy. The suffix names which executable
  to version-probe for a crate that does not install one named after itself
  (`ignorefile-cli` ships `ign` and `ignorefile`); it changes nothing about
  what is installed, so folding it into `<toolSetHash>` would make adding
  `:ign` to a working workflow miss the entry it already has — paying a full
  rebuild to stop paying full rebuilds, and splitting the cache between two
  spellings of one request from then on. The published `cargo-tools` output
  excludes it for the parallel reason: consumers hash that output to reproduce
  this key, so the two have to agree on what a tool set is.

  Dropping the toolchain from the key is only sound because rustup's own shims
  are excluded from the archive — otherwise the layer would carry `rustc` and
  `cargo` under a key that never mentions which toolchain they came from, and a
  restore would overwrite the installed toolchain with an older one.
  `binPaths` therefore emits a `!<bin>/<shim>` and `!<bin>/<shim>.exe` pair for
  each of the fourteen names in `RUSTUP_SHIMS`, plus the same directory
  negations `buildPaths` needs. See the warning about proving this in
  `CLAUDE.md`: a test asserting a shim is merely absent from disk after a
  restore proves nothing, because rustup puts every shim back itself.

The layers are split by how often each one invalidates, which is the whole
point of separating them: a `rustc` bump invalidates `build` but neither
`registry` nor `bin`, so splitting them stops a compiler upgrade from forcing a
re-download of every crate it never touched or a recompile of every tool it
does not affect.
...
- **Exclusions are negation globs, never deletion — and they only work on a
  files-only manifest.** This is the shape a layer archives under
  `cache-prune: off`; under the default `safe` it archives a stage instead,
  described below. `buildPaths` (`src/cache/paths.ts`) emits
...
### Pruning Filters the Manifest, and Staging Is What Makes It Possible

A cache entry's identity is not its key alone. `getCacheVersion` in
`@actions/cache` hashes `paths.join("|")` together with the compression method
and a salt, and an entry is matched on `(key, version)` — so **changing the
paths array changes which entry you are asking for**. That is the constraint
every pruning design has to answer, and answering it wrong is silent: the save
succeeds, writes a real entry, and no later restore can ever find it, because
the restoring job computes its version from a paths array it has not yet
learned the content of.

So pruning cannot narrow the paths array. Instead:

1. **Attribution.** `readFingerprints` (`src/cache/prune.ts`) reads the
   `<name>-<hash>` directories cargo already writes under
   `target/<profile>/.fingerprint/` and recovers a hash-to-package mapping from
   them. This is authoritative where `Swatinem/rust-cache` strips a trailing
   `-$hash` off a filename and guesses.
2. **Keep-set.** `computeKeepSet` intersects that mapping with the packages
   `cargo metadata --format-version 1 --locked` still resolves to
   (`src/cache/metadata.ts`), yielding the files worth archiving.
3. **Staging.** `buildStageRoots`/`registryStageRoot` name one fixed directory
   per tree, and `stagePaths` hands **that alone** to the client. The array is
   therefore identical in every job regardless of what the keep-set contains.
   `stageFiles` hard-links the keep-set into it before a save; `unstageFiles`
   moves it back out after a restore. Hard links rather than copies, because
   they share an inode and therefore an mtime, and mtime is what cargo's
   freshness check reads — a copy would look newer than the source it was
   built from and invalidate everything.

Three properties follow, and each has a guard because losing it is silent:

- **Nothing is ever deleted from the working tree.** The keep-set is a
  save-time filter, so a wrong keep-set costs a wrong-sized archive. A post
  step that deleted from `target/` would run after the job's real work, so the
  same mistake would surface as a build that succeeded and a checkout now
  missing artifacts — and on a self-hosted runner the damage outlives the job.
  Both inputs are outside our control, and cargo concedes the fingerprint
  format has no stability guarantee.
- **An empty or unusable keep-set falls back to the coarse globs.** Every
  failure mode converges on the same symptom — nothing resolved, therefore
  nothing attributable, therefore zero files — and saving that is not a small
  cache but a poisoned one: an entry that exists, hits its key, restores
  nothing, and leaves every later job rebuilding while believing it was warm.
  `computeKeepSet` refuses to mark such a set usable, `filesToStage` falls back
  to walking the whole tree when it is not, and `stageLayers` drops any layer
  that staged zero files rather than saving an empty archive.
- **Pruning is skipped when it would not pay.** `PRUNE_WORTH_IT` in
  ...
  5% of the bytes. Measured rather than guessed: resolution runs about 1.5 ms
  per kept entry, so an explicit manifest costs _more_ the less there is to
  prune — a churned tree dropped 46% of 220 MB in 495 ms, an unchurned one
  spent 904 ms to drop 0.2%.
...
nothing imports and the coverage gate does not measure (see `AGENTS.md` →
...
The exemption is earned adapter by adapter, not inherited by living in the
file. `@actions/cache` earns it for its SDK and its network code, the
`cargo metadata` reader for shelling out, the crates.io client for its
`fetch`. The `node:fs` adapters earned none of those and moved to
`src/cache/fs.ts`, where `fs.test.ts` holds them to a real temp directory —
after a hand-written fake that disagreed with `readdirSync` about a missing
directory cost every pruned layer its save, unnoticed by a suite at 100%.
