...
The barrel `src/lib.ts` re-exports all twenty-one library modules; only its edge
to `src/action.ts` is drawn, since the other twenty would say nothing but "the
