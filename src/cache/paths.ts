...
export function registryPaths(
  ...
  // Without a resolved package set — `cache-prune: off`, or any of the ways
  // resolution can fail — the three bare directories are archived by tar's own
  // recursion, exactly as before. An empty set means nothing resolved, and
  // naming no crate at all would save a registry layer that restores none of
  // them, so it takes the same path.
  ...
  //
  // Expressed as an inclusion per resolved package rather than as a negation
  // per unresolved one, which means no directory has to be read to decide: an
  // unresolved crate is dropped simply by nothing naming it. Cargo package
  // names are restricted to alphanumerics, `-` and `_`, so none of them can
  // carry glob metacharacters.
  //
  // `registry/cache` itself must NOT stay in the list. It is a directory, and
  // `@actions/cache` runs `tar --files-from` with no `--no-recursion`, so
  // leaving it would re-expand the whole cache and readmit every crate these
  // patterns were selected to replace — the same trap the build layer's
  // trailing-slash negation exists to close.
