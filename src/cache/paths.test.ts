...
describe("registryPaths with a package set", () => {
  const packages = new Set(["serde@1.0.219", "cfg-if@1.0.0"]);

  // Version-exact, unlike the build layer. A `.crate` file is named
  // `<name>-<version>.crate`, so the version is right there in the filename —
  // where the build layer's fingerprint directories record only `<name>-<hash>`
  // and force attribution by name alone.
  it("names each resolved crate archive exactly", () => {
    const paths = registryPaths("/home/runner/.cargo", packages);
    expect(paths).toContain(
      "/home/runner/.cargo/registry/cache/**/serde-1.0.219.crate",
    );
    expect(paths).toContain(
      "/home/runner/.cargo/registry/cache/**/cfg-if-1.0.0.crate",
    );
  });

  // The point of the phase, on this layer: a crate that left the lockfile stops
  // being carried. It is expressed as an absence — an unresolved crate simply
  // has no pattern naming it — which is why the next test pins that the bare
  // directory is gone, since leaving it would readmit everything.
  it("stops naming the cache directory, which would readmit every crate", () => {
    const paths = registryPaths("/home/runner/.cargo", packages);
    expect(paths).not.toContain("/home/runner/.cargo/registry/cache");
  });

  // The index is never pruned. It is what makes a `.crate` resolvable at all,
  // it is shared across every package, and it is regenerated by a network
  // round-trip rather than from anything on disk.
  it("always keeps the index and the git database", () => {
    const paths = registryPaths("/home/runner/.cargo", packages);
    expect(paths).toContain("/home/runner/.cargo/registry/index");
    expect(paths).toContain("/home/runner/.cargo/git/db");
  });
  ...
  it("never archives registry/src or git/checkouts, with or without a set", () => {
    for (const paths of [
      registryPaths("/home/runner/.cargo"),
      registryPaths("/home/runner/.cargo", packages),
    ]) {
  ...
  // Same fallback rule as the build layer, and for the same reason: an empty
  // package set means nothing resolved, and naming no crate at all would save a
  // registry layer that restores none of them.
  it("falls back to the bare directories when the set is empty", () => {
    expect(registryPaths("/c", new Set())).toEqual(registryPaths("/c"));
  });

  it("reproduces the previous behaviour when no set is given", () => {
